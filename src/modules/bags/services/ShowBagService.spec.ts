...
import ShowBagService from './ShowBagService';
...
describe('ShowBag', () => {
  beforeEach(async () => {
    ...
    showBag = new ShowBagService(fakeBagsRepository);
  ...
  it('should be able to show an existent bag', async () => {
    const bag = await fakeBagsRepository.create({
      user: undefined,
      prices: undefined,
    });

    expect(await showBag.execute(bag.id)).toEqual(bag);
  });

  it('should not be able to show a non existent bag', async () => {
    const id = new ObjectID();

    await expect(showBag.execute(id)).rejects.toBeInstanceOf(AppError);
  });
});
