...
import ListAssociatedOriginalExamsService from './ListAssociatedOriginalExamsService';
...
let listAssociatedOriginalExams: ListAssociatedOriginalExamsService;
...
describe('ListAssociatedOriginalExams', () => {
  ...
  beforeEach(async () => {
    ...
    listAssociatedOriginalExams = new ListAssociatedOriginalExamsService(
      fakeOriginalExamsRepository,
    );
  ...
  it('should be able to list all original exams already associated with an exam id', async () => {
    fakeOriginalExamsRepository.create({
      title: 'not-associated-exam',
      lab_id: 'any-lab',
      exam_original_id: 'original-id',
      lab_id_exam_original_id: 'any-laboriginal-id',
    });

    const associatedOriginalExams = await listAssociatedOriginalExams.execute();

    expect(associatedOriginalExams).toEqual([originalExam1, originalExam2]);
  });
});
